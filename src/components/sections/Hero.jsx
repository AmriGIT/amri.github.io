...
const Hero = () => {
  const fullstackLabel = getDurationLabel(new Date(2016, 5), new Date(2022, 9));
  const backendLabel = getDurationLabel(new Date(2022, 8));
  const heroParagraphs = HERO_CONTENT.replace("{{BACKEND_EXPERIENCE}}", backendLabel).split("\n").filter(Boolean);
  const stats = [
    { value: backendLabel, label: "Backend Java experience since 2022" },
    { value: "20+", label: "Projects and product features" },
    { value: "3", label: "Core focus: API, data, UI" },
  ];
        ...
        <div>
          ...
          <motion.div variants={fadeIn(0.36)} initial="hidden" animate="visible" className="mt-8 flex flex-wrap gap-3">
            ...
            <a href="#projects" className="inline-flex items-center justify-center gap-2 rounded-lg border border-white/10 bg-white/[0.03] px-5 py-3 text-sm font-semibold text-white transition hover:border-amber-200/50 hover:bg-amber-200/10 hover:text-amber-100 focus:outline-none focus:ring-2 focus:ring-amber-200/40">
              See Projects <FaArrowDown className="text-xs" />
            </a>
          </motion.div>
          ...
          <div className="overflow-hidden rounded-lg border border-white/10 bg-white/[0.04] shadow-2xl shadow-black/40">
            ...
            <div className="grid gap-3 border-t border-white/10 bg-zinc-950/90 p-5 sm:grid-cols-2">
              <div>
                ...
                <p className="mt-2 text-sm font-medium text-white">June 2016 - Oct 2022</p>
              ...
              <div>
                ...
                <p className="mt-2 text-sm font-medium text-white">Sept 2022 - Present</p>
